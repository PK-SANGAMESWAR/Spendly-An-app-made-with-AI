---
description: Runs parallel security and quality code review for a specific expense-tracker feature. Pass the spec name as argument e.g. /code-review-feature 03-login
...
If `.gemini/specs/$ARGUMENTS.md` does not exist, stop immediately and say:
"Spec file not found at .gemini/specs/$ARGUMENTS.md. Please check the spec name and try again."
...
**expense-tracker-security-reviewer** receives:
...
**expense-tracker-quality-reviewer** receives:
- The combined diff from the pre-flight check
- Spec file for context: `.gemini/specs/$ARGUMENTS.md`
...
Security Findings
[expense-tracker-security-reviewer output]
Quality Findings
[expense-tracker-quality-reviewer output]
...
- Do NOT proceed if the spec file at `.gemini/specs/$ARGUMENTS.md` does not exist — report it and stop
