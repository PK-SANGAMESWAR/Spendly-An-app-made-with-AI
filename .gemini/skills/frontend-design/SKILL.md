...
Spendly flashes are rendered inside the `<main>` block in `base.html` using the categories `success`, `error`, and `info`. Design styles for these categories in `style.css` using semantic colors:
- `success`: soft green background with dark green text and border
- `error`: soft red background with dark red text and border
- `info`: soft blue background with dark blue text and border
- Keep alert boxes padded with `12px 16px`, rounded corners (`var(--radius-sm)` or `8px`), and a subtle close button or fading/dismissal effect if appropriate.
...
### 2. The code
- **Template file(s)** - full Jinja2 with `{% extends "base.html" %}` and a `{% block content %}` unless building `base.html` itself. Use Jinja control flow (`{% for %}`, `{% if %}`) with standard context variable names.
  Always use these standard variable names when passing data from routes to templates:
  - `expenses`     → list of expense rows (dicts or Row objects)
  - `user`         → current user dictionary/Row (id, name, email)
  - `categories`   → list of category strings (e.g. `["Food", "Travel", ...]`)
  - `total_spend`  → float representing the total sum of expenses
  - `stats`        → dictionary of computed metrics (e.g. `total_spent`, `transaction_count`, `top_category`)
...
Fintech dashboards must never fail silently on empty datasets. When `expenses` is empty or there is no transaction history:
- Do not display an empty table. Instead, show an empty state illustration or icon (e.g., `receipt` or `file-text` in a muted color), a friendly heading (e.g., "No expenses tracked yet"), and a call-to-action button (e.g., "Add your first expense").
- Center-align empty states with generous vertical padding (e.g., `48px` or `64px`) to create breathing room.
- Use the pattern:
  ```html
  {% if expenses %}
    <!-- render table -->
  {% else %}
    <div class="empty-state">
      ...
      <p>Add your expenses to see detailed category analysis and budget breakdowns.</p>
...
Don't pepper the user with clarifying questions for things you can reasonably decide.
- **Ask about:** layout type (e.g., standalone page vs modal), data source/integration quirks, major user flows.
- **Don't ask about:** color choices, icon selection, spacing, font sizes, markup tags.
